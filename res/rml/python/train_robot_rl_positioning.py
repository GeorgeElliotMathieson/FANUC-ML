...
class RobotPositioningEnv(gym.Env):
    ...
    def __init__(self, gui=True, gui_delay=0.0, workspace_size=0.7, clean_viz=False, viz_speed=0.0, verbose=False, parallel_viz=False, rank=0, offset_x=0.0):
        ...
        if self.verbose and self.rank == 0:
            print(f"\n{'='*60}\nWORKSPACE BOUNDS COLLISION PERMANENTLY DISABLED\n{'='*60}\n")
        
        # Initialize collision tracking
        # Remove consecutive collision counters since we're using fixed penalties
        # self.consecutive_ground_collisions = 0
        # self.consecutive_bounds_collisions = 0
        self.collision_penalty_base = 1.0  # Fixed penalty of 1 point
        # Remove the exponential penalty calculation
        # self.collision_penalty_exponent = 1.2
        self.max_collision_penalty = 1.0  # Cap is now the same as the base
        
        # Set ground plane height to 0.0 to use the standard ground plane as the boundary
        self.ground_plane_height = 0.0  # Standard ground plane height (no offset)
        
        # Add state tracking for collision events
        self.in_ground_collision = False  # Track if we're currently in a ground collision
        self.in_bounds_collision = False  # Track if we're currently in a bounds collision (will always be False)
        
        self.ground_collision_threshold = 0.01  # Threshold for ground collision detection
        self.workspace_bounds_margin = 0.05  # Margin for workspace bounds collision detection (not used)
        ...
        # Define action space - NORMALIZED JOINT POSITIONS AND SPEEDS
        # Each action value represents a percentage (-100 to 100) of the joint's range
        # First 6 values are joint positions, next 6 values are joint speeds
        self.action_space = spaces.Box(
            low=-100.0,  # -100% of joint range
            high=100.0,  # 100% of joint range
            shape=(12,),  # 6 joint positions + 6 joint speeds as percentages of their ranges
            dtype=np.float32
        )
        ...
        # - Normalized distance to target (1) - between -100 and 100
        # - Normalized joint positions (6) - between -100 and 100
        # - Previous action (12) - between -100 and 100 (6 positions + 6 speeds)
        self.observation_space = spaces.Box(
            low=-np.inf,  # Some values like direction can still be negative
            high=np.inf,
            shape=(32,),  # Updated to include normalized joint positions and previous actions (12)
            dtype=np.float32
        )
        
        # Initialize previous action with zeros for both positions and speeds
        self.previous_action = np.zeros(12)
        ...
        self.timeout_steps = 50  # Changed from 200 to 50
    ...
    def step(self, action):
        ...
        # Split the action into positions and velocities
        # First 6 values are positions, next 6 values are velocities
        ...
        for i in range(len(joint_positions)):
            if i in self.robot.joint_limits:
                ...
                # Scale the normalized action (-100 to 100) to the joint's range
                # First convert from -100 to 100 scale to 0 to 1 scale
                # Use full floating-point precision without rounding
                normalized_action = (joint_positions[i] + 100.0) / 200.0
                
                # Then scale to the joint's range
                # Use full floating-point precision without rounding
                joint_position = limit_low + normalized_action * (limit_high - limit_low)
                
                # Add to target positions
                target_joint_positions.append(joint_position)
            else:
                # For joints without limits, use a default position
                target_joint_positions.append(0.0)
        
        # Process joint velocities
        target_joint_velocities = []
        for i in range(len(joint_velocities)):
            # Scale the normalized action (-100 to 100) to a reasonable velocity range
            # First convert from -100 to 100 scale to -1 to 1 scale
            normalized_velocity = joint_velocities[i] / 100.0
            
            # Scale to a reasonable velocity range (-5 to 5 rad/s)
            max_velocity = 5.0  # Maximum joint velocity in rad/s
            joint_velocity = normalized_velocity * max_velocity
            
            # Add to target velocities
            target_joint_velocities.append(joint_velocity)
        
        # Convert to numpy arrays
        target_joint_positions = np.array(target_joint_positions)
        target_joint_velocities = np.array(target_joint_velocities)
        
        # Store the normalized action for the next observation
        self.previous_action = action.copy()
        
        # Apply action to the robot - pass both positions and velocities
        self.robot.step((target_joint_positions, target_joint_velocities))
    ...
    def _get_observation(self):
        ...
        # Normalize joint positions to -100 to 100 range based on joint limits
        ...
        ee_position = state[12:15]
        ee_orientation = state[15:19]
        
        # Calculate relative position (target - ee)
        relative_position = self.target_position - ee_position
        
        # Calculate distance to target (scalar)
        distance_to_target = np.linalg.norm(relative_position)
        
        # Normalize distance to target based on workspace size and scale to -100 to 100
        normalized_distance = distance_to_target / self.workspace_size
        normalized_distance = normalized_distance * 200.0 - 100.0
        # Cap extreme values to prevent numerical instability
        normalized_distance = max(min(normalized_distance, 100.0), -100.0)
        
        # Direction to target (normalized)
        direction_to_target = relative_position / (distance_to_target + 1e-6)  # Avoid division by zero
        
        # Combine all observations into a single array
        observation = np.concatenate([
            ee_position,              # End effector position (3)
            ee_orientation,           # End effector orientation (4)
            self.target_position,     # Target position (3)
            direction_to_target,      # Direction to target (3)
            [normalized_distance],    # Normalized distance to target (1) - now -100 to 100
            normalized_joint_positions, # Normalized joint positions (6) - now -100 to 100
            self.previous_action      # Previous action (12) - between -100 and 100 (6 positions + 6 speeds)
        ])
...
class CustomActorCriticNetwork(nn.Module):
    def __init__(self, feature_dim):
        ...
        # Use powers of 2 for layer sizes - more principled approach
        # Gradually decrease size from input to output
        input_dim = 32  # Updated observation dimension (includes previous action with 12 values now)
        
        # Shared feature extractor with consistent size reduction
        self.shared_net = nn.Sequential(
            nn.Linear(input_dim, 512),
            nn.ReLU(),
            # Add batch normalization for standardization
            nn.BatchNorm1d(512),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.BatchNorm1d(256),
        ...
        # Output layers - 12 actions for the robot's 6 motors (position and speed for each)
        # Use tanh activation scaled to -100 to 100 range
        # Each output represents the percentage of the motor's range
        self.mean_layer = nn.Sequential(
            nn.Linear(64, 12),  # Updated to 12 outputs: 6 for positions + 6 for speeds
        ...
        self.log_std_layer = nn.Linear(64, 12)  # Updated to 12 outputs to match mean layer
    ...
    def forward(self, x):
        ...
        mean = self.mean_layer(actor_features)  # Now outputs 12 values between -100 and 100
        log_std = self.log_std_layer(actor_features)  # Now outputs 12 log std values
        # No clipping to ensure full floating-point precision
